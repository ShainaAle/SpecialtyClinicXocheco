...
foreach ($rolesCatalog as $roleRow) {
    $roleNamesById[(int)$roleRow['id_tipo_usuario']] = $roleRow['nombre_rol'];
}
        ...
        if ($nombre === '' || $apellidos === '' || $correo === '' || $idTipoUsuario <= 0 || $calle === '' || $numeroExterior === '' || $colonia === '' || $codigoPostal === '' || $ciudad === '' || $estado === '') {
            ...
            try {
                ...
                if ($userId <= 0) {
                    ...
                    $roleName = $roleNamesById[$idTipoUsuario] ?? '';
                    if ($roleName === 'Paciente') {
                        $redirectTarget = '../pacientes/?user=' . $newUserId;
                    } elseif ($roleName === 'Médico') {
                        $redirectTarget = '../medicos/?user=' . $newUserId;
                    }
                }
