...
$availableUsers = patientRowsRecepcion(
    ...
    'i',
    [(int)($editing['id_usuario'] ?? 0) ?: $selectedUserId]
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete') {
        ...
        $adeudo = $patientId > 0 ? (int)($_POST['adeudo'] ?? 0) : 0;
        $birthDate = DateTimeImmutable::createFromFormat('Y-m-d', $fechaNacimiento);
                    ...
                    <div class="mb-3">
                        ...
                        <select name="id_usuario" class="form-select">
                            ...
                            <?php foreach ($availableUsers as $user): ?>
                                <option value="<?php echo (int)$user['id_usuario']; ?>" <?php echo ((int)($editing['id_usuario'] ?? 0) === (int)$user['id_usuario'] || (!$editing && $selectedUserId === (int)$user['id_usuario'])) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($user['nombre_completo'] . ' · ' . $user['correo']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <?php if (!$editing && $selectedUserId > 0): ?>
                        <div class="alert alert-info">Este usuario ya está seleccionado para completar su ficha de paciente.</div>
                    <?php endif; ?>
                    <div class="mb-3">
                        <label class="form-label">Fecha de nacimiento</label>
                        <input type="date" name="fecha_nacimiento" max="<?php echo date('Y-m-d'); ?>" class="form-control" value="<?php echo htmlspecialchars($editing['fecha_nacimiento'] ?? ''); ?>">
                    </div>
                    <div class="mb-3">
